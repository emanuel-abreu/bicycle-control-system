...
TotemService = require('../services/TotemService')
...
class TrancaController {
    ...
    integrarNaRede = async(request, response) => {
        ...
        const totem = await TrancaService.getTotem(idTotem);
        const tranca = await TrancaService.getTranca(idTranca);

        if (!totem.success) return response.status(Number(totem.failure.codigo)).json([totem.failure]);
        if (!(tranca.success.status in validTrancaStatuses)) return request.status(422).json(criarErro('422', Messages.TRANCA_ESTADO_INCOMPATIVEL));
    ...
    retirarDaRede = async(request,response) => {
        ...
        if (!(statusAcaoReparador in validTrancaStatuses)) return response.status(422).json(criarErro('422', Messages.STATUS_INVALIDO));
